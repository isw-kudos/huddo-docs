...
| `app.env.BANNED_LINK_URL_EXPRESSION`      | **Optional**: A JavaScript regular expression string where matching link (Bookmark) URLs will be denied creation. Existing links that match will cause errors when updating any links on a node, unless all matching links are deleted.  Default: `^javascript:` |
